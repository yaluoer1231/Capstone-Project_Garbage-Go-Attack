...
public class Floor3 : MonoBehaviour {
    ...
    void Start()
    {
        gameObject.transform.position = new Vector3(-1.62f, -1.08f, -10);
    }
    ...
    void Update()
    {
        try
        {
            ...
            if (pos.x > -1.62 && pos.x < 1.62 && pos.y > -1.08 && pos.y < 1.19)
            {
                transform.position = pos;
            }
            else if (pos.x < -1.62 || pos.x > 1.62)
            {
                if (pos.y < -1.08 || pos.y > 1.19)
                {
